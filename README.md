...
2. This workflow starts with the taxonomically assigned ESVs from the default dataflow and generates operational taxonomic units (OTUs) based on 97% sequence similarity.
...
5. This workflow starts with Illumina paired-end reads that do NOT to overlap.  If you would like to process the R1 and R2 files separately this pipeline can be used with caution.  Note that if your reads overlap, they should be processed using the default pipeline because longer sequences tend to make better taxonomic assignments with higher accuracy.
...
conda activate MetaWorks_v1.9.2

# On the GPSC activate using source
source ~/miniconda/bin/activate MetaWorks_v1.9.2
```
...
The pipeline requires ORFfinder 0.4.3 available from the NCBI at ftp://ftp.ncbi.nlm.nih.gov/genomes/TOOLS/ORFfinder/linux-i64/ .  This program should be downloaded, made executable, and put in your conda environment path (ex. ~/miniconda/envs/MetaWorks_v1.9.2/bin).

```linux
# go to your conda environment bin
cd ~/miniconda3/envs/MetaWorks_v1.9.2/bin/.
...
wget https://github.com/terrimporter/MetaWorks/releases/download/v1.9.2/MetaWorks1.9.2.zip
unzip MetaWorks1.9.2.zip
cd MetaWorks1.9.2
...
Begin by downloading the latest MetaWorks release available at https://github.com/terrimporter/MetaWorks/releases/tag/v1.9.2 by using wget from the command line:

```linux
# download the pipeline
wget https://github.com/terrimporter/MetaWorks/releases/download/v1.9.2/MetaWorks1.9.2.tar.gz
...
Create then activate the MetaWorks_v1.9.2 environment:
...
source ~/miniconda3/bin/activate MetaWorks_v1.9.2
...
```linux
cd ~/miniconda3/envs/MetaWorks_v1.9.2/lib
ln -s ../glibc-2.14/lib/libc.so.6 libc.so.6
```

Create the shell script file LD_PATH.sh in the following location to set the environment variable: ~/miniconda3/envs/MetaWorks_v1.9.2/etc/conda/activate.d/LD_PATH.sh
...
~/miniconda3/envs/MetaWorks_v1.9.2/etc/conda/deactivate.d/LD_PATH.sh
...
Teresita M. Porter. (2020, June 25). MetaWorks: A Multi-Marker Metabarcode Pipeline (Version v1.9.2). Zenodo. http://doi.org/10.5281/zenodo.4741407 
